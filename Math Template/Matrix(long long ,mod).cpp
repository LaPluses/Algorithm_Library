       ...
       for(int j = 0 ; j < x.c ; ++ j)
        {
           ...
           for(int k = 0 ; k < c ; ++ k)
           {
                long long pval = getval(ele[i][k]) * getval(x.ele[k][j]);
                if(pval >= modx) pval %= modx;
             	sum = sum +  pval;
   ...
   {      
      ...
      for(int i = 0 ; i < r ; ++ i)
       for(int j = 0 ; j < x.c ; ++ j)
        {
           long long sum = 0;
           for(int k = 0 ; k < c ; ++ k) sum += ele[i][k]*x.ele[k][j];
           res.ele[i][j] = getvals(sum);
        }
      return res;
   }
